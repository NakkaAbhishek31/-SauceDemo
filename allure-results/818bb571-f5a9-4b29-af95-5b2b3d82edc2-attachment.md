...
```ts
  ...
  66  | 
  67  |     await loginPage.login('standard_user', 'secret_sauce');
  68  | 
  69  |     await expect(inventoryPage.pagetitile).toBeVisible();
  70  | 
  71  |     await navigation.clickOnOpenMenu();
  72  |     await navigation.clickOnLogout();
  73  | 
  74  |     await expect(loginPage.loginButton).toBeVisible();
  75  | 
  76  |     await page.goto('/inventory.html');
  77  | 
  78  |     await expect(loginPage.loginButton).toBeVisible();
  ...
  151 | 
  152 | 
  153 | test.only('Reset App State clears every selected product button', async ({ page,loginPage, inventoryPage,navigation }) => {
  154 | 
  ...
  158 |     let poductnames=await inventoryPage.productNames.allTextContents();
  159 | 
  160 |     for(let productname of poductnames)
  161 |     {
  162 |       await inventoryPage.addProductToCart(productname);
  163 |     }
  164 | 
> 165 |     await expect(inventoryPage.cartBadge).toHaveCount(poductnames.length);
      |                                           ^ Error: expect(locator).toHaveCount(expected) failed
  166 | 
  167 |     await navigation.clickOnOpenMenu();
  168 | 
  169 |     await navigation.clickOnRestAppState();
  170 | 
  171 |     await expect(inventoryPage.cartBadge).toBeHidden();
  172 | 
  173 |     // await expect(
  174 |     //     inventoryPage.productCard(firstproduct)
  175 |     //         .getByRole('button', { name: /Remove/i })
  176 |     // ).toBeVisible();
  177 | 
  178 | });
```